...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float(*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int myCol)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	if (values_count > 0)
	{
		...
		ImU32 out;
		if (myCol == 0)
			out = ((ImU32)IM_F32_TO_INT8_SAT(1)) << IM_COL32_R_SHIFT;
		else
			out = ((ImU32)IM_F32_TO_INT8_SAT(0)) << IM_COL32_R_SHIFT;

		if (myCol == 1)
			out |= ((ImU32)IM_F32_TO_INT8_SAT(1)) << IM_COL32_G_SHIFT;
		else
			out |= ((ImU32)IM_F32_TO_INT8_SAT(0)) << IM_COL32_G_SHIFT;

		if (myCol == 2)
			out |= ((ImU32)IM_F32_TO_INT8_SAT(1)) << IM_COL32_B_SHIFT;
		else
			out |= ((ImU32)IM_F32_TO_INT8_SAT(0)) << IM_COL32_B_SHIFT;
		out |= ((ImU32)IM_F32_TO_INT8_SAT(1)) << IM_COL32_A_SHIFT;
		//const ImU32 col_base = GetColorU32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines : ImGuiCol_PlotHistogram);
		const ImU32 col_base = out;
...
//TODO
void ImGui::PlotHistogram(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride, int myCol)
{
	ImGuiPlotArrayGetterData data(values, stride);
	PlotEx(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size, myCol);
}
