...
Sun::Sun(float s, glm::vec3 color) 
{ 
...
void Sun::Icosahedron(Mesh & mesh) 
{ 
...
uint32_t subdivideEdge2(uint32_t f0, uint32_t f1, const glm::vec3 &v0, const glm::vec3 &v1, Mesh &io_mesh, std::map<Edge, uint32_t> &io_divisions) 
{ 
...
void SubdivideMesh2(const Mesh &meshIn, Mesh &meshOut) 
{ 
  ...
  { 
    ...
    const uint32_t f3 = subdivideEdge2(f0, f1, v0, v1, meshOut, divisions); 
    const uint32_t f4 = subdivideEdge2(f1, f2, v1, v2, meshOut, divisions); 
    const uint32_t f5 = subdivideEdge2(f2, f0, v2, v0, meshOut, divisions); 
...
{ 
  ...
  //SubdivideMesh2(m2, m3); 
  //SubdivideMesh2(m3, m4); 
  SubdivideMesh2(m2, m); 
